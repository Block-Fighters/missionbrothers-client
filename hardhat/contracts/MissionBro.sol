...
contract MissionContract is Ownable, ReentrancyGuard {
    ...
    enum RewardDistributionMethod { TopThree, Fair }
    ...
    function distributeRewards(uint256 _missionId) public {
        ...
        require(mission.creator != address(0), "Mission does not exist");
        require(mission.isClosed == false, "Mission is closed");
        require(block.timestamp > mission.missionEndTime, "Mission is still active");
        ...
        if (mission.rewardMethod == RewardDistributionMethod.TopThree) {
            rewards = distributeRewardsTopThree(participants, participantsCount, totalRewards);
        } else if (mission.rewardMethod == RewardDistributionMethod.Fair) {
            rewards = distributeRewardsFair(participants, participantsCount, totalRewards);
        } else {
    ...
    function distributeRewardsTopThree(address[] storage, uint256 participantsCount, uint256 totalRewards
    ...
    // 모든 참가자에게 균등하게 보상 분배
    function distributeRewardsFair(address[] storage, uint256 participantsCount, uint256 totalRewards
    ) internal pure returns (uint256[] memory) {
        uint256[] memory rewards = new uint256[](participantsCount);
